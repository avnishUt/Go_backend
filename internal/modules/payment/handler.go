...
func (h Handler) Create(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		response.Error(c, apperrors.BadRequest("invalid payment payload", err.Error()))
		return
	}
	...
	response.OK(c, http.StatusCreated, "payment created", result)
}

func (h Handler) CompleteMockUPI(c *gin.Context) {
	if !h.available(c) {
		return
	}
	var req CompleteMockUPIRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.Error(c, apperrors.BadRequest("invalid payment payload", err.Error()))
		return
	}
	result, err := h.service.CompleteMockUPI(c.Request.Context(), c.Param("id"), req)
	if h.handleError(c, err) {
		return
	}
	response.OK(c, http.StatusOK, "mock upi payment completed", result)
}
...
func (h Handler) Get(c *gin.Context) {
	...
	response.OK(c, http.StatusOK, "payment fetched", result)
}
...
func (h Handler) List(c *gin.Context) {
	...
	response.OK(c, http.StatusOK, "payments fetched", result)
}
